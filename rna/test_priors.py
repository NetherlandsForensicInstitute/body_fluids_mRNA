...
def test_priors(nfolds, tc):
    ...
    # ======= Initialize =======
    emtpy_numpy_array = np.zeros((nfolds, len(settings.binarize), len(settings.softmax), len(settings.models), len(settings.priors)-1))
        ...
        for i, binarize in enumerate(settings.binarize):
            ...
            for j, softmax in enumerate(settings.softmax):
                for k, models in enumerate(settings.models):
                    ...
                    # if settings.augment:
                    #     accuracies['train'][n, i, j, k] = calculate_accuracy(model, mle, augmented_data, target_classes)
                    # else:
                    #     accuracies['train'][n, i, j, k] = calculate_accuracy(model, mle, augmented_data, target_classes)
                    # accuracies['test'][n, i, j, k] = calculate_accuracy(model, mle, augmented_data, target_classes)
                    # accuracies['test as mixtures'][n, i, j, k] = calculate_accuracy(model, mle, augmented_data, target_classes)
                    ...
                    for t, target_class in enumerate(target_classes):
                        for p, priors in enumerate(settings.priors):
                            if priors != None:
                                ...
                                target_class_str = vec2string(target_class, label_encoder)
                                cllr_test[target_class_str][n, i, j, k, p] = cllr(lrs_after_calib[str_prior][:, t], augmented_data[str_prior].y_test_nhot_augmented, target_class)
                                cllr_test_as_mixtures[target_class_str][n, i, j, k, p] = cllr(
                                    lrs_test_as_mixtures_after_calib[str_prior][:, t], augmented_data[str_prior].y_test_as_mixtures_nhot_augmented, target_class)
                                cllr_mixtures[target_class_str][n, i, j, k, p] = cllr(lrs_after_calib_mixt[str_prior][:, t], y_nhot_mixtures, target_class)
        ...
        if key != 'None':
            ...
            lrs_before_calib_mixt_i, lrs_after_calib_mixt_i = \
                perform_analysis(n, binarize, softmax, models, mle, label_encoder, X_train_augmented,
                                 y_train_nhot_augmented, X_calib_augmented, y_calib_nhot_augmented, X_test_augmented,
                                 y_test_nhot_augmented, X_test_as_mixtures_augmented, X_mixtures, target_classes,
                                 name=key, save_hist=False)
...
def plot_scatterplot_lrs(lrs, label_encoder, y_nhot_mixtures, target_classes, show=None, savefig=None):
    ...
    for method, data in lrs.items():
        ...
        plot_scatterplot_lr(data.lrs_after_calib_mixt, label_encoder, y_nhot_mixtures, y,
