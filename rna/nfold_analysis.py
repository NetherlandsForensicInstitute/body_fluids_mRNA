...
def nfold_analysis(nfolds, tc):
            ...
            for j, softmax in enumerate(settings.softmax):
                ...
                for k, models in enumerate(settings.models):
                    ...
                    for t, target_class in enumerate(target_classes):
                        target_class_str = vec2string(target_class, label_encoder)
                        print(target_class_str)
                        cllr_test[target_class_str][n, i, j, k] = cllr(lrs_after_calib[:, t], y_test_nhot_augmented, target_class)
                        print("Cllr is: {}".format(cllr(lrs_after_calib[:, t], y_test_nhot_augmented, target_class)))
    ...
    # plot_boxplot_of_metric(accuracies['train'], "accuracy")
    # plot_boxplot_of_metric(accuracies['test'], "accuracy")
    # plot_boxplot_of_metric(accuracies['test as mixtures'], "accuracy")
    # plot_boxplot_of_metric(accuracies['mixture'], "accuracy")
    # plot_boxplot_of_metric(accuracies['single'], "accuracy")
