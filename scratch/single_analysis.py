...
def individual_analysis(tc, show=True):
    ...
    if settings.augment:
        ...
        # lrs_before_calib, lrs_after_calib, lrs_before_calib_mixt, lrs_after_calib_mixt= \
        #     generate_lrs(model, mle, X_train_nrp, y_train_nrp, X_calib_nrp, y_calib_nrp, X_test_augmented, X_mixtures, target_classes)
    ...
    if settings.augment:
        ...
        # TODO: Uncomment for original data when ignore repeated measurements
        # print("Single train accuracy nrp (original): {0:.4f}".format(get_accuracy(model, mle, y_train_nrp, X_train_nrp)))
        # print("Test accuracy (augmented): {0:.4f}".format(get_accuracy(model, mle, y_test_nhot_augmented, X_test_augmented)))
        # print("Mixture accuracy (original): {0:.4f}".format(get_accuracy(model, mle, y_nhot_mixtures, X_mixtures)))
        # y_test_nrp = y_test_nrp.reshape(-1, 1)
        # print("Single test accuracy nrp (original): {0:.4f}\n".format(get_accuracy(model, mle, mle.inv_transform_single(y_test_nrp), X_test_nrp)))
